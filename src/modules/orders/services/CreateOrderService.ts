...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const formattedProductList = products.map(product => {
      const productEntity = productsList.find(
        productFromList => productFromList.id === product.id,
      );

      if (!productEntity) {
        throw new AppError(`Product id ${product.id} not found`);
      }

      if (product.quantity > productEntity.quantity) {
        throw new AppError(
          `Not enough ${productEntity.name} in the stock. Quantity available: ${productEntity.quantity}`,
        );
      }
